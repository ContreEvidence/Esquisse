...
for(const item of items){
  ...
  const signals=norm(`${item.c} ${item.k} ${item.n}`);
  const highStakes=/fiscal|jurid|droit|succession|transmission|sante|securite travail|credit|emprunt|meuble tourisme|courte duree|copropriete/.test(signals);
  if(highStakes){
    const externalLinks=[...html.matchAll(/<a\s+[^>]*href=["']https?:\/\/[^"']+["']/gi)];
    if(!externalLinks.length)warnings.push(`${item.h}: sujet sensible sans lien externe de source détecté.`);
  }
  if(/\bdoit\b|\bdevez\b|\bil faut acheter\b|\bil faut vendre\b/i.test(strip(html))&&/invest|placement|allocation|action|etf|immobilier/.test(signals)){
    warnings.push(`${item.h}: formulation potentiellement prescriptive à relire dans un contenu patrimonial.`);
  }
}
