...
for(const rel of ['assets/personal-space.js','assets/finance-cockpit.js','assets/finance-architecture.js','assets/cockpit-progressive.js']){
...
for(const rel of htmlFiles()){
  ...
  if(rel==='mon-espace.html'){
    ...
    html=html.replace(/<link\s+rel="stylesheet"\s+href="assets\/cockpit-progressive\.css(?:\?[^"']*)?"\s*\/?>/i,'');
    html=html.replace(/<script\s+src="assets\/cockpit-progressive\.js(?:\?[^"']*)?"\s*><\/script>/i,'');
    html=html.replace(/<section\s+class="fc-section"\s+data-ce-finance-legal="1"[\s\S]*?<\/section>/i,'');
    html=html.replace(/<\/head>/i,`${architectureCss}${progressiveCss}</head>`);
    html=html.replace(/<div class="fc-shell">/i,`<div class="fc-shell">${guardrail}`);
    html=html.replace(/<\/body>/i,`${architectureJs}${progressiveJs}</body>`);
  }
  fs.writeFileSync(file,html,'utf8');count++;
}
console.log(`Mon espace intégré à ${count} pages ; cockpit en divulgation progressive, version ${SITE_VERSION}.`);
