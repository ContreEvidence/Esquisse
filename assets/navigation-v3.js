(() => {
  const VERSION = '20260806-4';
  const currentScript = document.currentScript;
  const baseUrl = new URL('.', currentScript?.src || window.location.href);
  const cssUrl = new URL(`navigation-tabs.css?v=${VERSION}`, baseUrl).href;

  document.querySelectorAll('link[data-ce-navigation]').forEach(link => link.remove());
  const stylesheet = document.createElement('link');
  stylesheet.rel = 'stylesheet';
  stylesheet.href = cssUrl;
  stylesheet.dataset.ceNavigation = VERSION;
  document.head.appendChild(stylesheet);

  const header = document.querySelector('header');
  if (!header) return;

  const nestedPage = /\/(articles|themes)\//.test(window.location.pathname);
  ...
  header.innerHTML = `
    ...
    <nav id="ce-navigation" class="ce-nav" aria-label="Navigation principale">
      <div class="container ce-tabs">
        <div class="ce-tab-group" data-section="commencer">
          <button class="ce-tab ce-tab-start" type="button" aria-expanded="false"><span>Commencer</span><span class="ce-chevron" aria-hidden="true"></span></button>
          <div class="ce-panel"><div class="ce-panel-inner ce-panel-3">
            ...
            <a class="ce-panel-link" href="${href('parcours-argent.html')}"><strong>Organiser mon argent</strong><small>Épargne, supports et allocation.</small></a>
          </div></div>
        </div>
        <div class="ce-tab-group" data-section="travail">
          <button class="ce-tab ce-tab-work" type="button" aria-expanded="false"><span>Vie professionnelle</span><span class="ce-chevron" aria-hidden="true"></span></button>
          <div class="ce-panel"><div class="ce-panel-inner ce-panel-3">
            ...
            <a class="ce-panel-link" href="${href('bibliotheque.html?theme=travail')}"><strong>Explorer la bibliothèque</strong><small>Filtrer les contenus professionnels.</small></a>
          </div></div>
        </div>
        <div class="ce-tab-group" data-section="argent">
          <button class="ce-tab ce-tab-money" type="button" aria-expanded="false"><span>Argent</span><span class="ce-chevron" aria-hidden="true"></span></button>
          <div class="ce-panel"><div class="ce-panel-inner ce-panel-3">
        ...
        <div class="ce-tab-group" data-section="comprendre">
          <button class="ce-tab ce-tab-think" type="button" aria-expanded="false"><span>Décider & comprendre</span><span class="ce-chevron" aria-hidden="true"></span></button>
          <div class="ce-panel"><div class="ce-panel-inner ce-panel-3">
        ...
        <div class="ce-tab-group" data-section="bibliotheque">
          <button class="ce-tab ce-tab-library" type="button" aria-expanded="false"><span>Bibliothèque</span><span class="ce-chevron" aria-hidden="true"></span></button>
          <div class="ce-panel"><div class="ce-panel-inner ce-panel-4">
  ...
  const groups = [...header.querySelectorAll('.ce-tab-group')];
  ...
  const closeGroups = except => {
    groups.forEach(group => {
      ...
      group.querySelector('button.ce-tab')?.setAttribute('aria-expanded', 'false');
    });
  };

  groups.forEach(group => {
    const trigger = group.querySelector('button.ce-tab');
    if (!trigger) return;
  ...
  mobileButton?.addEventListener('click', () => {
    ...
    mobileButton.setAttribute('aria-label', willOpen ? 'Fermer le menu' : 'Ouvrir le menu');
  ...
  header.querySelectorAll('[data-section]').forEach(item => {
    const paths = patterns[item.dataset.section] || [];
    if (paths.some(path => currentPath.endsWith(path))) item.classList.add('is-current');
  });
  ...
  window.addEventListener('resize', () => {
    if (window.innerWidth >= 700) header.classList.remove('nav-open');
  });
})();
