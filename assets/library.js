(() => {
  ...
  const aliases = {
    ...
    investir:'investissement allocation etf actions pea placement capital crédit analyse rendement capitalisation',investissement:'investir allocation etf actions pea placement capital crédit analyse rendement capitalisation',rendement:'capitalisation intérêts composés frais investissement placement',capitalisation:'rendement intérêts composés frais long terme investissement',interets:'capitalisation composés rendement placement',composes:'intérêts capitalisation rendement long terme',frais:'rendement net capitalisation etf fonds gestion',passive:'etf indiciel frais gestion rendement',active:'fonds gestion frais rendement',pilotee:'gestion frais etf rendement',world:'msci monde etf actions',sp500:'s&p 500 etf actions usa',credit:'dette remboursement emprunt taux investissement capacité mensualité simulateur',emprunt:'crédit capacité mensualité taux immobilier banque simulateur',capaciteemprunt:'crédit emprunt mensualité taux immobilier simulateur',mensualite:'crédit emprunt capacité taux immobilier simulateur',rembourser:'crédit dette remboursement anticipé investir',
    immobilier:'logement résidence locatif crédit acheter louer travaux vendre conserver scpi foncière',logement:'immobilier résidence acheter louer crédit vendre',acheter:'immobilier logement résidence crédit',vendre:'immobilier bien conserver capital rendement',conserver:'immobilier bien vendre capital rendement',scpi:'immobilier indirect liquidité frais patrimoine rendement',fonciere:'immobilier indirect bourse patrimoine',
    ...
    outil:'simulateur calculateur modèle tableur',simulateur:'outil calculateur emprunt crédit capacité rendement',calculateur:'outil simulateur tableur',tableur:'outil calculateur modèle',
    ...
    biais:'décision contradiction preuve hypothèse vérification scénario',confirmation:'décision contradiction preuve hypothèse vérification',taux:'probabilité scénario base crédit rendement emprunt simulateur',base:'probabilité scénario décision analyse',
  ...
  const typeButtons = [...tools.querySelectorAll('[data-type]')];
  ...
  function scoreCard(card,query){
    ...
    const title=normalise(card.querySelector('h3')?.textContent),chip=normalise(card.querySelector('.theme-chip')?.textContent),desc=normalise(card.querySelector('p')?.textContent),tags=normalise(card.dataset.tags),full=normalise(fullTextIndex[card.dataset.href] || ''),phrase=normalise(query),tokens=queryTokens(query);
    let score=0;
    if(title===phrase)score+=180;else if(title.startsWith(phrase))score+=130;else if(title.includes(phrase))score+=90;
    if(chip.includes(phrase))score+=45;if(tags.includes(phrase))score+=45;if(desc.includes(phrase))score+=30;if(full.includes(phrase))score+=22;
    tokens.forEach(token=>{if(title.includes(token))score+=24;if(chip.includes(token))score+=12;if(tags.includes(token))score+=14;if(desc.includes(token))score+=7;if(full.includes(token))score+=3;});
