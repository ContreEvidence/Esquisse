(() => {
  ...
  style.textContent = `
    ...
    .ce-follow-dialog{position:relative;width:min(620px,94vw);max-height:min(760px,90vh);overflow:auto;border:1px solid rgba(212,171,86,.55);border-radius:20px;background:#10161b;color:#fff;box-shadow:0 30px 80px rgba(0,0,0,.45);padding:1.4rem}
    ...
    @media(max-width:759px){.ce-flat-actions{gap:.38rem}.ce-follow-trigger{min-height:40px;padding:.45rem .68rem}.ce-follow-dialog{padding:1.1rem}.ce-follow-feed{align-items:flex-start;flex-direction:column}.ce-follow-copy{width:100%}}
  `;
  ...
  backdrop.innerHTML = `
    <section class="ce-follow-dialog" role="dialog" aria-modal="true" aria-labelledby="ce-follow-title">
      ...
      <h2 id="ce-follow-title">Suivre Contre-Évidence</h2>
      <p class="ce-follow-intro">Choisissez ce que vous voulez suivre. Les nouveaux contenus apparaissent automatiquement dans votre lecteur RSS, sans compte et sans algorithme.</p>
      <div class="ce-follow-section">
        <h3>Par RSS</h3>
        <p>Copiez le flux de votre choix dans Feedly, Inoreader ou votre lecteur habituel.</p>
      ...
      <div class="ce-follow-section">
        ...
        <p>Pour découvrir les formats courts et les nouveaux dossiers.</p>
        <div class="ce-follow-socials">${socials.map(([name,url]) => `<a href="${url}" target="_blank" rel="noopener noreferrer">${name}</a>`).join('')}</div>
      </div>
      <div class="ce-follow-note"><strong>Version e-mail :</strong> elle sera ajoutée ici avec double confirmation dès que le service d’envoi sera relié. Le RSS est déjà entièrement automatique.</div>
    </section>`;
  ...
  function openDialog() {
    ...
    const preferred = backdrop.querySelector(`[data-feed="${preferredFeed()}"]`);
  ...
  function addTrigger() {
    ...
    if (foot && !foot.querySelector('.ce-follow-trigger')) {
      ...
      button.type = 'button';
      button.className = 'ce-follow-trigger';
      button.textContent = 'Suivre Contre-Évidence';
      button.addEventListener('click', openDialog);
      foot.prepend(button);
    }
  }
